...
float xKp = 0.09; // 0.1 was good starting point    // 0.09                                                  
float xKi = 0.01;  // 0.005                         // 0.01                             
float xKd = 0.030;   // 0.02                        // 0.030
...
// PID parameters
double xSetpoint, xInput, xOutput, x_angle; //for X
double ySetpoint, yInput, yOutput, y_angle; //for Y
...
RunningMedian samplesX = RunningMedian(21);
RunningMedian samplesY = RunningMedian(21);
...
void loop() {
  ...
  samplesX.add(xRead);
  samplesY.add(yRead);
  xFinal = samplesX.getMedian();
  yFinal = samplesY.getMedian();
  ...
  xInput = map(yFinal,264,730,-123,123); // Map these according to our touchscreen dimensions
  yInput = map(xFinal,262,732,-93,93); // 271.27 x 205.74 or 248.87 x 187.40  
...
//  }
  ...
  x_angle = map(xOutput, -15.0, 15.0, SERVOMIN, SERVOMAX);
  y_angle = map(yOutput, -15.0, 15.0, SERVOMIN, SERVOMAX);

  LobotSerialServoMove(Serial, ID1, x_angle-16, 0);
  LobotSerialServoMove(Serial, ID2, y_angle+34, 0);
